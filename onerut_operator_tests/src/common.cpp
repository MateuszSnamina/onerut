...
std::shared_ptr<onerut_typed_operator::AbstractOperator<unsigned>> first_compound_operator() {
    ...
    auto op1 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(3.2, 2, 1);
    auto op2 = std::make_shared<onerut_typed_operator::DiagOperator<unsigned>>(5.5, 1);
    auto op3 = std::make_shared<onerut_typed_operator::DiagOperator<unsigned>>(1.9, 2);
    auto op4 = std::make_shared<onerut_typed_operator::DiagOperator<unsigned>>(7, 0);
    OpPtrT op_first_arg = op1;
    std::vector<OpPtrT> op_other_argv({op2, op3, op4});
    std::vector<char> op_opv({'-', '+', '-'});
    auto op = std::make_shared<onerut_typed_operator::OpPlusMinusOperator<unsigned>>(op_first_arg, op_other_argv, op_opv);
...
std::shared_ptr<onerut_typed_operator::AbstractOperator<unsigned>> second_compound_operator() {
    using OpT = onerut_typed_operator::AbstractOperator<unsigned>;
    using OpPtrT = std::shared_ptr<const OpT>;
    auto op1 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(2.1, 2, 1);
    auto op2 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(1.1, 2, 3);
    auto op3 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(1.3, 2, 0);
    auto op4 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(2.3, 2, 1);
    auto op5 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(3.1, 2, 3);
    auto op6 = std::make_shared<onerut_typed_operator::HopOperator<unsigned>>(3.3, 2, 3);
    auto op7 = std::make_shared<onerut_typed_operator::DiagOperator<unsigned>>(1.7, 2);
    OpPtrT op_first_arg = op1;
