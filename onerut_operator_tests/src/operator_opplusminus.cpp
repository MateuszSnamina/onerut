...
TEST(operator_op_plus_minu, test_2) {
    ...
    using OpPtrT = std::shared_ptr<const OpT>;
    const auto op1 = std::make_shared<onerut_operator::HopOperator<unsigned>>(3.2, 2, 1);
    const auto op2 = std::make_shared<onerut_operator::ZeroOperator<unsigned>>();
    const OpPtrT op_first_arg = op1;
...
TEST(operator_op_plus_minu, test_3) {
    ...
    using OpPtrT = std::shared_ptr<const OpT>;
    const auto op1 = std::make_shared<onerut_operator::ZeroOperator<unsigned>>();
    const auto op2 = std::make_shared<onerut_operator::HopOperator<unsigned>>(3.2, 2, 1);
    const OpPtrT op_first_arg = op1;
    const std::vector<OpPtrT> op_other_argv{op2};
    const std::vector<char> op_opv{'+'};
    const auto op = std::make_shared<onerut_operator::OpPlusMinusOperator<unsigned>>(op_first_arg, op_other_argv, op_opv);
...
TEST(operator_op_plus_minu, test_4) {
    ...
    using OpPtrT = std::shared_ptr<const OpT>;
    const auto op1 = std::make_shared<onerut_operator::HopOperator<unsigned>>(3.2, 2, 1);
    const auto op2 = std::make_shared<onerut_operator::DiagOperator<unsigned>>(5.5, 1);
    const auto op3 = std::make_shared<onerut_operator::DiagOperator<unsigned>>(1.9, 2);
    const auto op4 = std::make_shared<onerut_operator::DiagOperator<unsigned>>(7, 0);
    const OpPtrT op_first_arg = op1;
...
TEST(operator_op_plus_minu, test_5) {
    using OpT = onerut_operator::AbstractOperator<unsigned>;
    using OpPtrT = std::shared_ptr<const OpT>;
    const auto op1 = std::make_shared<onerut_operator::HopOperator<unsigned>>(2.1, 2, 1);
    const auto op2 = std::make_shared<onerut_operator::HopOperator<unsigned>>(1.1, 2, 3);
    const auto op3 = std::make_shared<onerut_operator::HopOperator<unsigned>>(1.3, 2, 0);
    const auto op4 = std::make_shared<onerut_operator::HopOperator<unsigned>>(2.3, 2, 1);
    const auto op5 = std::make_shared<onerut_operator::HopOperator<unsigned>>(3.1, 2, 3);
    const auto op6 = std::make_shared<onerut_operator::HopOperator<unsigned>>(3.3, 2, 3);
    const auto op7 = std::make_shared<onerut_operator::DiagOperator<unsigned>>(1.7, 2);
    const OpPtrT op_first_arg = op1;
