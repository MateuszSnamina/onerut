...
inline
std::optional<onerut_parser_exec::Asset> inuput_2_asset(const std::string onerut_inuput) {
    ...
    // #########################################################################
    const auto _ast_asset_head = _ast_source_head->compile(std::make_shared<onerut_parser_rules::CompilerRulesConcrete>());
    // -------------------------------------------------------------------------
    if (onerut_verbose) {
        const auto ast_asset_ast_chart = _ast_asset_head->to_ast_chart(onerut_parser_rules::asset_to_esc_data);
        onerut_parser_exec::print_ast_chart(std::cout, _parsed_x3_info.input, ast_asset_ast_chart, "[test][common][asset][diagram]");
    }
    // #########################################################################
    onerut_parser_exec::Asset _asset = _ast_asset_head->asset;
    // --------------------------------------------------    
    if (onerut_verbose)
        onerut_parser_rules::print_receipt(std::cout, _asset, "[test][common] ");
    // --------------------------------------------------    
    return _asset;
}
